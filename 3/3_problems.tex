...
\begin{s}{3.22}
    ...
    the equivalence classes in the quotient space. Since $q^{-1}$ is continuous, we can 
    map the points of $q(U)$ back to $X$, which is just $\bigcup_{g\in G} g(U)$. Since $G$ is an action by homeomorphisms,
    each $g(U)$ is open, so the union is open. Thus since $q^{-1}(q(U))$ is open, we have that 
    $q^(U)$ is open, thus $q$ is an open mapping.\\
